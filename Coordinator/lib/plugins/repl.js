...
module.exports = function(arcus, port){
  ...
  arcus.on('start', function(){
    _server = net.createServer(function(socket){
       ...
       * Reset stats
       */
      ctx.resetStats = function(){
        arcus.resetStats();
        socket.writeln('Stats reset.');
      }
      ...
      ctx.stats = function(){
        ...
        socket.writeln('');
        socket.row('Arcus uptime', Helper.formatLapsed(arcus.uptime[0]));
        socket.row('Process uptime', Helper.formatLapsed(arcus.uptime[1]));
        socket.writeln('');
        socket.row('Connections', arcus.connections.length);
        socket.row('Groups', arcus.groups.length);
        socket.writeln('');
        socket.row('Dropped Connections', arcus.stats.droppedConnections);
        socket.row('Dropped Groups', arcus.stats.droppedGroups);
        socket.row('Dropped Handshakes', arcus.stats.droppedHandshakes);
        socket.writeln('');
      }
