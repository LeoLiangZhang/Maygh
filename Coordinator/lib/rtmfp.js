...
var RTMFP = module.exports = function(){
  ...
  this.readPacket = function(pkt){
    ...
    if (RTMFP_VALID_MARKERS.indexOf(marker) == -1){
      throw new Error('Ivalid packet marker ' + marker);
    }
  ...
  this.readMessage = function(pkt, lastMessage){
    ...
    switch(type)
    {
      ...
      case 0x71:
        ...
        //TODO: read IPv6 also
        while(clippedPkt.available() > 0 && clippedPkt.readInt8() ) {
      ...
      // TODO: handle 91 2E F8 CE 3B 05 09 E2 B6 10 00 04 03 03 02 01 -> Asking for ack
      //
      case 0x10 :
      case 0x11 :
        message = {};
        ...
        if(message.sequence == 0x02)
        {
          ...
          //TODO: Add by Liang: To fix reading NET_CONNECTION_RESPONSE
          var a = message.unknown2[0], b = message.unknown2[1], c = message.unknown2[2];
//          console.log(a,b,c);
          if(a == 0x02){
        	  message.unknown2 = [0x00, 0x14, 0x00, 0x00, 0x00, 0x01];
        	  clippedPkt.readBytes(3);
          }
          if(a == 4 && b == 0 && c == 0){
        	  NET_CONNECTION_ADDRESSES_RESPONSE = 100;
        	  message.type = NET_CONNECTION_ADDRESSES_RESPONSE;
        	  return message;

          }
          var __pos = clippedPkt.pos();
          if(a == 0x14 && b == 0x00){ //COMMAND response
        	  clippedPkt.pos(__pos - 1);
//        	  console.log(clippedPkt.peek());
          }
          // END, Liang
  ...
  this.writePacket = function(pkt, message){
    ...
    switch(message.type)
    {
      ...
      case RTMFP.NET_CONNECTION_REQUEST:
        ...
        pkt.writeBytes([0x02, 0x01, 0x01]); //hardcoded for testing sequence, stage, command index
  ...
  // TODO: Added by Liang: to access _timeNow for other commands;
  this._timeNow = _timeNow;
  this.writeSize = writeSize;
  this.nativeRTMFP = _rtmfp;
