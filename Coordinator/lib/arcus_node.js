        ...
        if(typeof __p.commands.get(message.commandName) === 'object') {
          ...
          (function(handler, nc, message, arcus){
            process.nextTick(function() {
            	// Changed by Liang: to support multi-server
              var result = handler.method.apply(handler.context, [nc].concat(message.commandData, message));
              if(result == ArcusNode.ASYNCOMMANDRESULT)
            	  return;
    	  ...
    	  if(_self.cloud){
    		  ...
    		  var peerNc;
    	        for(var peerNum = 0; peerNum < message.peerIds.length; peerNum++){
    	          ...
    	          var raw_pid = message.peerIds[peerNum];
    	          var sname = _self.cloud.name;

    	          if(!peerNc){
    	        	  raw_pid = message.peerIds[peerNum];
    	        	  var pid = _self.cloud.helper.formatPeerID(raw_pid);
    	        	  peerNc = _self.cloud.dict_pid_connections[pid];
    	        	  if(!peerNc)
    	        		  __p.logger.warn("RENDEZVOUZ_REQUEST","Peer Not Found", pid);
    	        	  else{
    	        		  sname = peerNc.sname;
    	        		  peerNc = peerNc.nc;
    	        	  }
...
//    	            if(sname != _self.cloud.name)
    	            _self.cloud.informNewcomer(sname, raw_pid, message.tag, remoteInfo);
    	            ...
    	            if(peerNc.addresses.length > 0 && newcomerMessage.address != undefined)
    	            {
    	              ...
    	              send(directResponse, peerNc.addresses[0]);
    	            }
      ...
      case RTMFP.ACK:
        ...
        if(nc.__p.lastMessage && nc.__p.lastMessage.sequence == message.sequence
  ...
  this.informNewcomer = function(peerId, tag, remoteInfo){
	  var peerNc = __p.peerIdMap.get(peerId);
	  if(!peerNc) return;

	  //Inform peer about newcomer
      var newcomerMessage = { type: RTMFP.RENDEZVOUZ_NEWCOMER};
      newcomerMessage.tag = tag;
      newcomerMessage.peerId = peerNc.peerId;
...
ArcusNode.ASYNCOMMANDRESULT = new Object();
